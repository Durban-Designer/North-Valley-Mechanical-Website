<template>
  <div class="main">
    ...
    <appointmentSet class="appointmentSet" v-else-if="appointmentShow" v-on:saveAppointment="newAppointment" v-on:backAppointment="hideAppointment"></appointmentSet>
    <div class="newAppointmentState" v-else>
      <div class="month">
      ...
      <ul class="days">
        ...
        <li v-bind:class="{'sundayCol': true, 'dayBooked': true, 'rowOne': sunday.row === 1, 'rowTwo': sunday.row === 2, 'rowThree': sunday.row === 3, 'rowFour': sunday.row === 4, 'rowFive': sunday.row === 5, 'rowSix': sunday.row === 6}" v-for="sunday in sundays">{{sunday.day}}</li>
      </ul>
      <div>{{ this.day }}</div>
...
<style scoped lang="less">
  .main {
    ...
    grid-template-rows: 200px repeat(6, 50px);
  }
